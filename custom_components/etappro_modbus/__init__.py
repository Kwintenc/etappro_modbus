...
class AlfenModbusHub:
    ...
    async def read_modbus_data_station(self):
        status_data = await self.read_holding_registers(self._address,1100,4)
        if status_data.isError():
            _LOGGER.error("Modbus read failed for 1100")
            return False
        else:
            for i, value in enumerate(status_data.registers):
                _LOGGER.debug("Register %s = %s", 1100 + i, value)
        ...
        status_data = self.read_holding_registers(self._address,1106,4)
        if status_data.isError():
            _LOGGER.error("Modbus read failed for 1106")
            return False
        else:
            for i, value in enumerate(status_data.registers):
                _LOGGER.debug("Register %s = %s", 1106 + i, value)
            
        self.data["evPlugTemperature"] =  round(self.decode_from_registers(status_data.registers,0,2,self._client.DATATYPE.FLOAT32),2)
        self.data["gridPlugTemperature"] =  round(self.decode_from_registers(status_data.registers,2,2,self._client.DATATYPE.FLOAT32),2)
        return True
    ...
    async def read_modbus_data_socket(self,socket):
        ...
        if(socket == 1):
            voltage_data = await self.read_holding_registers(socket,306,6)
            if voltage_data.isError():
                _LOGGER.error("Modbus read failed for 306")
                return False
            else:
                for i, value in enumerate(voltage_data.registers):
                    _LOGGER.debug("Register %s = %s", 306 + i, value)
            ...
            self.data["socket_"+str(socket)+"_VL1-N"] =   round(self.decode_from_registers(voltage_data.registers,0,2,self._client.DATATYPE.FLOAT32),2)
            self.data["socket_"+str(socket)+"_VL2-N"] =   round(self.decode_from_registers(voltage_data.registers,2,2,self._client.DATATYPE.FLOAT32),2)
            self.data["socket_"+str(socket)+"_VL3-N"] =   round(self.decode_from_registers(voltage_data.registers,4,2,self._client.DATATYPE.FLOAT32),2)
            ...
            current_data = await self.read_holding_registers(socket,320,8)
            if current_data.isError():
                _LOGGER.error("Modbus read failed for 320")
                return False
            else:
                for i, value in enumerate(current_data.registers):
                    _LOGGER.debug("Register %s = %s", 320 + i, value)
            
            self.data["socket_"+str(socket)+"_currentL1"] =   round(self.decode_from_registers(current_data.registers,0,2,self._client.DATATYPE.FLOAT32),2)
            self.data["socket_"+str(socket)+"_currentL2"] =   round(self.decode_from_registers(current_data.registers,2,2,self._client.DATATYPE.FLOAT32),2)
            self.data["socket_"+str(socket)+"_currentL3"] =  round(self.decode_from_registers(current_data.registers,4,2,self._client.DATATYPE.FLOAT32),2)
            self.data["socket_"+str(socket)+"_currentSum"] =   round(self.decode_from_registers(current_data.registers,6,2,self._client.DATATYPE.FLOAT32),2)
            ...
            power_data = await self.read_holding_registers(socket,338,8)
            if power_data.isError():
                _LOGGER.error("Modbus read failed for 338")
                return False
            else:
                for i, value in enumerate(power_data.registers):
                    _LOGGER.debug("Register %s = %s", 338 + i, value)
            
            self.data["socket_"+str(socket)+"_realPowerL1"] =   round(self.decode_from_registers(power_data.registers,0,2,self._client.DATATYPE.FLOAT32),2)
            self.data["socket_"+str(socket)+"_realPowerL2"] =   round(self.decode_from_registers(power_data.registers,2,2,self._client.DATATYPE.FLOAT32),2)
            self.data["socket_"+str(socket)+"_realPowerL3"] =   round(self.decode_from_registers(power_data.registers,4,2,self._client.DATATYPE.FLOAT32),2)
            self.data["socket_"+str(socket)+"_realPowerSum"] =   round(self.decode_from_registers(power_data.registers,6,2,self._client.DATATYPE.FLOAT32),2)    
            ...
            energy_data = await self.read_holding_registers(socket,374,4)
            if energy_data.isError():
                _LOGGER.error("Modbus read failed for 374")
                return False
            else:
                for i, value in enumerate(energy_data.registers):
                    _LOGGER.debug("Register %s = %s", 374 + i, value)
            
            self.data["socket_"+str(socket)+"_realEnergyDeliveredSum"] =   round(self.decode_from_registers(energy_data.registers,0,4,self._client.DATATYPE.FLOAT64),2) 
            ...
            status_data = await self.read_holding_registers(socket,1200,12)
            if status_data.isError():
                _LOGGER.error("Modbus read failed for 1200")
                return False
            else:
                for i, value in enumerate(status_data.registers):
                    _LOGGER.debug("Register %s = %s", 1200 + i, value)
            ...
            extra_status_data = await self.read_holding_registers(socket,1214,2)
            if extra_status_data.isError():
                _LOGGER.error("Modbus read failed for 1214")
                return False
            else:
                for i, value in enumerate(extra_status_data.registers):
                    _LOGGER.debug("Register %s = %s", 1214 + i, value)
            
            self.data["socket_"+str(socket)+"_setpointAccounted"] =  self.decode_from_registers(extra_status_data.registers, 0, 1,self._client.DATATYPE.UINT16)    
            self.data["socket_"+str(socket)+"_chargephases"] =  self.decode_from_registers(extra_status_data.registers, 1, 1,self._client.DATATYPE.UINT16) 
    ...
    async def read_modbus_data_product(self):
        identification_data = await self.read_holding_registers(self._address, 100, 40)
        if identification_data.isError():
            _LOGGER.error("Modbus read failed for 100")
            return False
        else:
            for i, value in enumerate(identification_data.registers):
                _LOGGER.debug("Register %s = %s", 100 + i, value)
        ...
        more_identification_data = await self.read_holding_registers(self._address, 157, 22)
        if more_identification_data.isError():
            _LOGGER.error("Modbus read failed for 157")
            return False
        else:
            for i, value in enumerate(more_identification_data.registers):
                _LOGGER.debug("Register %s = %s", 157 + i, value)

        self.data["serial"] = self.decode_from_registers(more_identification_data.registers, 0, 11, self._client.DATATYPE.STRING).strip('\x00')

        year    = self.decode_from_registers(more_identification_data.registers, 11, 1,self._client.DATATYPE.INT16)
        month   = self.decode_from_registers(more_identification_data.registers, 12, 1,self._client.DATATYPE.INT16)
        day     = self.decode_from_registers(more_identification_data.registers, 13, 1,self._client.DATATYPE.INT16)
        hour    = self.decode_from_registers(more_identification_data.registers, 14, 1,self._client.DATATYPE.INT16)
        minute  = self.decode_from_registers(more_identification_data.registers, 15, 1,self._client.DATATYPE.INT16)
        second  = self.decode_from_registers(more_identification_data.registers, 16, 1,self._client.DATATYPE.INT16)
        uptime  = self.decode_from_registers(more_identification_data.registers, 17, 4,self._client.DATATYPE.UINT64)
        utcoffset = self.decode_from_registers(more_identification_data.registers, 21, 1,self._client.DATATYPE.INT16)
